...
export const TourCardButtons = ({ tourId }: { tourId: string }) => {
  ...
  const { data: purchasedResponse, isPending: isPurchasedPending } = useCheckPurchasedTour(tourId);
      ...
      <>
        <div className="flex flex-col w-full mt-6 space-y-4">
          <Button
            ...
            onClick={() => navigate(`/tours/view/${purchasedResponse?.data?.tour_id}`)}
          >
            Ver tour
          </Button>
        </div>
      ...
      <>
        <div className="flex flex-col w-full mt-6 space-y-4">
          ...
          <Button
            ...
            onClick={() => setCartItem({ id: tourId, isSelected: true, quantity: 1 })}
          >
            Agregar al carrito
          </Button>
        </div>
